...
public abstract class BaseKLFCommand {
     ...
     * Some KLF200 commands use a session to help with this determination. However, there are some that do not
     * use a session and therefore the only way to reconcile is to examine which node the command was executed for.
     *
     * @param responseCommand The response returned from the KLF200
     * @param data The payload returned in the command
     * @returns True if this command instance reconciles with the data received, false otherwise.
     */
    protected boolean canHandleNode(KLFGatewayCommands responseCommand, byte[] data) {
        byte responseNode = extractNode(responseCommand, data);
    ...
    protected byte extractNode(KLFGatewayCommands responseCommand, byte[] data) {
        throw new UnsupportedOperationException(
                "This commands claims it needs to extract node, but it didn't implement the method to do so.");
    }
     ...
     * However, some commands may result in more than one response payload.
     *
     * @param responseCommand The response command to handle
     * @param data Data received from the KLF unit.
     */
    ...
    protected int extractSession(KLFGatewayCommands responseCommand, byte[] data) {
        throw new UnsupportedOperationException(
                "This commands claims it needs to extract session, but it didn't implement the method to do so.");
    }
}